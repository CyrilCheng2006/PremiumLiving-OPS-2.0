...
namespace PremiumLivingOPS.Views.AfterService
{
    public partial class AccountReceivableForm : Form
    {
        ...
        // ── Invoice List refresh (embedded CARD 4) ───────────────────────
        internal void RefreshInvoiceGrid()
        {
            string kw = txtInvSearch?.Text.Trim();
        ...
        private void dgvInvoice_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (dgvInvoice.Columns[e.ColumnIndex].Name == "invColStatus" && e.Value != null)
            {
        ...
        // ── Record Payment (opened from KPI bar button or Invoice grid double-click)
        private void OpenRecordPayment()
        {
            // Determine selected invoice: prefer Invoice grid selection, else AR grid
            InvoiceDetailEntity inv = null;

            if (dgvInvoice.SelectedRows.Count > 0)
            {
                int idx = dgvInvoice.SelectedRows[0].Index;
            ...
            if (inv == null)
            {
                MessageBox.Show("Please select an invoice from the Invoice List below.",
